  ...
  return (
    ...
    >
      ...
      {(tableLabel || customerName) && (
        <div className="flex flex-wrap items-center gap-3 border-b border-zinc-800/60 px-4 py-2 text-sm">
          {tableLabel && (
        ...
        {order.order_items.map((item) => (
          <div key={item.id} className="py-1">
            <div className="flex items-start gap-2">
              <span className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-md bg-zinc-800 text-xs font-bold text-white">
                {item.quantity}
              </span>
                  ...
                  <div className="mt-0.5 space-y-0.5">
                    {item.modifiers_snapshot.map(
                      (mod: ModifierSnapshot, idx: number) => (
                        ...
                        >
                          <span className="text-zinc-600">{mod.group}:</span>{' '}
      ...
      {order.notes && (
        <div className="mx-4 mb-3 flex items-start gap-2 rounded-lg bg-zinc-800/60 px-3 py-2">
          ...
          <p className="text-xs text-amber-200/80 leading-snug">{order.notes}</p>
        </div>
      )}
